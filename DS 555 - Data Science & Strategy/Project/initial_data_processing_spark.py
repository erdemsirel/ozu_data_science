...
def initial_data_preparation(spark):
    ...
    print(df.columns)
    df = df.toPandas()
    df.to_csv("spark_processed_data.csv", index=False, sep="|")
    pd.Series(multi_choice_columns).to_csv("spark_multi_choice_columns.csv", index=False, sep="|")
    pd.DataFrame.from_dict(distinct_choice_lists_for_each_columns, orient="index").T.to_csv("spark_multi_choice_options.csv", index=False, sep="|")
    return df
